...
var CRMChat = (function () {
    ...
    var updateForwardSelectionUi = function () {
        ...
        $(".forward-target-item").each(function () {
            ...
            item.toggleClass(
                "active border-primary bg-light-primary",
                isSelected,
            );
    ...
    var renderReplyPreviewContent = function (content, type, message) {
        ...
        if (replyType === "sticker" || body.indexOf("[STICKER:") !== -1) {
            ...
            var stickerUrl =
                preferredUrl || (stickerMatch ? stickerMatch[1] : "");
        ...
        return (
            ...
            formatWhatsAppText(escapeHtml(body)) +
            "</div>"
        );
    };
    ...
    var renderComposerReplyPreview = function (content, type, message) {
        ...
        return (
            ...
            (mediaHtml
                ? '<div class="me-3 flex-shrink-0">' + mediaHtml + "</div>"
                : "") +
    ...
    var initCustomerList = function () {
        $(document).on("click", "#btn-save-message-edit", function () {
            ...
            $.post(appUrl + "chat/edit_message", { id: id, content: content })
                .done(function (response) {
                    if (response.success) {
                        ...
                        var msgElement = $("#messages-list [data-message-id='" + id + "']");
                        if (msgElement.length > 0) {
                            msgElement.replaceWith(renderMessage(response.data));
                        }
                    } else {
                ...
                .fail(function (xhr) {
                    ...
                    toastr.error(resp && resp.error ? resp.error : "Gagal mengubah pesan");
                })
                .always(function () {
        ...
        // WhatsApp Number Check - only allow digits
        $("#input-new-wa-number").on("input", function () {
            $(this).val($(this).val().replace(/[^0-9]/g, ""));
            // Reset result area when input changes
            $("#wa-check-result").addClass("d-none").html("");
        });

        // WhatsApp Number Check - Enter key
        $("#input-new-wa-number").on("keypress", function (e) {
            if (e.key === "Enter") {
                e.preventDefault();
                $("#btn-check-wa").trigger("click");
            }
        });

        // Check WhatsApp Number Button
        $("#btn-check-wa").on("click", function () {
            var phone = $("#input-new-wa-number").val().trim();
            if (!phone || phone.length < 8) {
                toastr.warning("Masukkan nomor WhatsApp yang valid (minimal 8 digit)");
                return;
            }
            ...
            $.ajax({
                url: appUrl + "chat/check_whatsapp",
                method: "GET",
                data: { phone: phone },
                success: function (response) {
                    var resultDiv = $("#wa-check-result");
                    resultDiv.removeClass("d-none");

                    if (response.success && response.on_whatsapp) {
                        // Number is on WhatsApp
                        var existingHtml = "";
                        if (response.existing_customer) {
                            existingHtml =
                                '<div class="alert alert-info d-flex align-items-center py-3 px-4 mb-3">' +
                                '<i class="ki-outline ki-information-4 fs-2 text-info me-3"></i>' +
                                '<div class="fs-7">Kontak sudah ada: <strong>' +
                                escapeHtml(response.existing_customer.name) +
                                "</strong></div></div>";
                        }

                        resultDiv.html(
                            '<div class="alert alert-success d-flex align-items-center py-3 px-4 mb-3">' +
                                '<i class="ki-outline ki-check-circle fs-2 text-success me-3"></i>' +
                                '<div class="fs-7"><strong>+' +
                                response.phone +
                                "</strong> terdaftar di WhatsApp</div></div>" +
                                existingHtml +
                                '<button type="button" class="btn btn-success w-100" id="btn-start-new-chat-wa" data-phone="' +
                                response.phone +
                                '" data-customer-id="' +
                                (response.existing_customer
                                    ? response.existing_customer.id
                                    : "") +
                                '">' +
                                '<i class="ki-outline ki-message-text-2 fs-4 me-2"></i> Mulai Chat</button>',
                        );
                    } else if (response.success && !response.on_whatsapp) {
                        // Not on WhatsApp
                        resultDiv.html(
                            '<div class="alert alert-danger d-flex align-items-center py-3 px-4 mb-0">' +
                                '<i class="ki-outline ki-cross-circle fs-2 text-danger me-3"></i>' +
                                '<div class="fs-7"><strong>+' +
                                response.phone +
                                "</strong> tidak terdaftar di WhatsApp. Tidak bisa memulai chat.</div></div>",
                        );
                    } else {
                        resultDiv.html(
                            '<div class="alert alert-warning d-flex align-items-center py-3 px-4 mb-0">' +
                                '<i class="ki-outline ki-shield-cross fs-2 text-warning me-3"></i>' +
                                "<div class=\"fs-7\">Gagal memverifikasi nomor. Pastikan WhatsApp Gateway aktif.</div></div>",
                        );
                    }
                },
                error: function () {
                    $("#wa-check-result")
                        ...
                        .html(
                            '<div class="alert alert-danger d-flex align-items-center py-3 px-4 mb-0">' +
                                '<i class="ki-outline ki-disconnect fs-2 text-danger me-3"></i>' +
                                "<div class=\"fs-7\">Gagal menghubungi server. Coba lagi nanti.</div></div>",
                        );
                },
                complete: function () {
        ...
        // Start New Chat from verified WA number
        $(document).on("click", "#btn-start-new-chat-wa", function () {
            var phone = $(this).data("phone");
            var existingId = $(this).data("customer-id");

            if (existingId) {
                // Customer exists, just open chat
                $("#modal_new_chat").modal("hide");
                selectCustomer(existingId, "individual");
                return;
            }

            // Create new customer then open chat
            var btn = $(this);
            btn.prop("disabled", true).html(
            ...
            $.ajax({
                url: appUrl + "chat/start_new_chat",
                method: "POST",
                data: { phone: phone },
                success: function (response) {
                    if (response.success) {
                        $("#modal_new_chat").modal("hide");
                        // Reset form
                        $("#input-new-wa-number").val("");
                        $("#wa-check-result").addClass("d-none").html("");
                        // Open new chat
                        toastr.success(
                            "Kontak baru berhasil dibuat: +" + phone,
                        );
                ...
                error: function () {
                    toastr.error("Gagal membuat kontak baru");
                    btn.prop("disabled", false).html(
                        '<i class="ki-outline ki-message-text-2 fs-4 me-2"></i> Mulai Chat',
                    );
                },
            });
        });

        // Reset new chat modal on close
        $("#modal_new_chat").on("hidden.bs.modal", function () {
            $("#input-new-wa-number").val("");
            $("#wa-check-result").addClass("d-none").html("");
            $("#search-new-chat").val("");
            $("#new-chat-list").html(
        ...
        $(document).on("click", "#btn-mark-all-read", function () {
            ...
            $.post(appUrl + "chat/bulk_mark_read", function (response) {
                if (response.success) {
                    ...
                    toastr.info(
                        response.message ||
                            "Tidak ada pesan baru untuk ditandai",
                    );
                }
            ...
            $.post(
                ...
                function (response) {
                    if (response.success) {
                        // Update UI for this specific item
                        if (contextTarget) {
                            var badgeWrapper = contextTarget.find(".unread-badge-wrapper");
                            if (badgeWrapper.length === 0) {
                                var row2 = contextTarget.find(".d-flex.justify-content-between.align-items-center").last();
                                var badgeHtml = '<div class="unread-badge-wrapper ms-2 d-flex flex-column align-items-end">' +
                                                '<span class="badge badge-circle badge-success whatsapp-green w-18px h-18px fs-9">1</span>' +
                                                '</div>';
                                row2.append(badgeHtml);
                            } else {
                                badgeWrapper.html('<span class="badge badge-circle badge-success whatsapp-green w-18px h-18px fs-9">1</span>');
                            }
                            
                            contextTarget.find(".name-text")
                                .removeClass("fw-semibold text-gray-800")
                                .addClass("fw-bold text-gray-900");
                            
                            contextTarget.find(".last-msg-text")
        ...
        $(document).on("click", "#menu-rename-chat", function (e) {
            ...
            $("#rename-chat-title").text(
                type === "group" ? "Ubah Nama Grup" : "Ubah Nama Kontak",
            );
        ...
        $(document).on("click", "#menu-assign-label", function (e) {
            ...
            if (!id || type === "group") {
                if (type === "group") toastr.warning("Label hanya tersedia untuk kontak individu");
                menu.hide();
                return;
            }
    ...
    var selectCustomer = function (customerId, type = "individual") {
        ...
        if (type === "group") {
            ...
            $("#btn-refresh-group").show(); // Show Refresh for Groups
            ...
            $("#btn-refresh-group").hide(); // Hide Refresh for Individuals
    ...
    var displayConversation = function (customer, messages, has_more) {
        ...
        if (type === "group") {
            $("#chat-header-initials")
                .html('<i class="bi bi-people-fill text-white fs-1"></i>')
            ...
            $("#chat-header-initials")
                .html('<i class="bi bi-person-fill text-white fs-1"></i>')
                .css("background-color", "#00a884")
                .removeClass("d-none");
            $("#chat-header-avatar-img").addClass("d-none");
        }

        // Auto-refresh avatar if missing
        if (!customer.avatar_url) {
            refreshAvatar(customer.id, type);
        }
    ...
    var renderMessage = function (msg) {
        ...
        if (msg.is_deleted == 1) {
            ...
            if (msg.is_edited == 1) {
                ...
                var editTime = (msg.updated_at || msg.created_at)
                    ...
                    .substring(0, 5);
                displayContent +=
                    '<span class="message-edited-label">Edited ' +
                    editTime +
                    "</span>";
        ...
        if (isOut) {
            ...
            if (msg.is_sending || msg.status === "sending") {
                senderLabel = "Anda";
            } else if (
                rawContent &&
            ...
            var iconClass =
                msg.status === "read"
                    ...
                    : msg.status === "delivered"
                      ...
                      : msg.status === "sending"
                        ? "bi bi-clock text-secondary"
                        : "bi bi-check2 text-gray-500";
            statusHtml =
                '<span class="message-status ' + iconClass + '"></span>';
        }
        ...
        var hasReply =
            msg.reply_message ||
            (msg.reply_content && msg.reply_content !== "");
        ...
        var bubbleHtml =
            ...
            msg.type +
            '">';
        var bubbleCloseHtml = "</div>";

        var loadingHtml = "";
        if (isOut && msg.is_sending) {
            loadingHtml =
                '<div class="message-loading-spinner">' +
                '<span class="spinner-border spinner-border-sm text-secondary"></span>' +
                "</div>";
            wrapperClass += " message-sending";
        }
        ...
        return (
            ...
            (msg.wa_message_id || "") +
            '" data-timestamp="' +
            (msg.wa_timestamp || msg.created_at_ts || 0) +
            '" data-user-id="' +
            (msg.user_id || 0) +
            '" data-created-at="' +
            msg.created_at +
        ...
        if (type === "video" || body.indexOf("[VIDEO:") !== -1) {
            ...
            if (match) {
                ...
                return (
                    ...
                    "</div>" +
                    (caption
                        ? '<div class="mt-1">' + escapeHtml(caption) + "</div>"
                        : "")
                );
            }
        }
    ...
    var getJakartaTimestamp = function () {
        return new Date()
    ...
    var sendMessage = function () {
        ...
        var tempMsg = {
            ...
            content: message,
            type: "text",
            created_at: getJakartaTimestamp(),
            created_at_ts: Math.floor(new Date().getTime() / 1000),
            wa_timestamp: Math.floor(new Date().getTime() / 1000),
            status: "sending",
            is_sending: true,
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    var tempMsgElement = $(
                        "#messages-list [data-message-id='" + tempId + "']",
                    );
                    if (tempMsgElement.length > 0) {
                        tempMsgElement.replaceWith(
            ...
            error: function () {
                toastr.error("Gagal mengirim pesan. Silakan coba lagi.");
                $("#messages-list [data-message-id='" + tempId + "']").remove();
                $("#message-input").val(message);
            },
        });
    };
    ...
    var initUploadPreviewEvents = function () {
        ...
        $(document).on("click", ".upload-emoji-btn", function (e) {
            ...
            emojis.forEach(function (e) {
                emojiHtml +=
                    '<span class="emoji-item" data-emoji="' +
                    e +
                    '">' +
                    e +
                    "</span>";
        ...
        $(document).on("click", "#send-preview-files", function () {
            ...
            pendingFiles.forEach(function (f, i) {
                ...
                if (forceAsDocument) {
                    ...
                    selectedFileType = f.type.startsWith("image/")
                        ? "image"
                        : "document";
                }
    ...
    var loadTemplates = function () {
        $.ajax({
            ...
            success: function (response) {
                if (response.success && response.grouped) {
                    ...
                    for (var cat in response.grouped) {
                        ...
                        html +=
                            '<div class="category-wrapper" data-cat="' + cat + '">';
                        html +=
                            '<div class="fw-bold text-muted fs-8 mb-3 mt-4 text-uppercase ls-1">' +
                            catName +
                            "</div>";
                        ...
                        response.grouped[cat].forEach(function (t) {
                            ...
                            html += '<div class="col-12 mb-1 template-container-item">';
                            html +=
                                '<div class="template-grid-item" data-content="' +
                                escapeHtml(t.content) +
                                '" data-search="' + escapeHtml((t.title || t.name) + " " + t.content).toLowerCase() + '" onclick="selectTemplate(this)">';
                            html +=
                            ...
                            html += "</div>";
                            html +=
                                '<div class="text-gray-600 fs-7 line-clamp-2">' +
                                escapeHtml(preview) +
                                "</div>";
                            html += "</div></div>";
                        });
                    ...
                    if (!html || Object.keys(response.grouped).length === 0) {
                        html =
                            '<div class="text-center text-muted py-10"><i class="ki-outline ki-folder-error fs-3x mb-3 d-block"></i><p>Belum ada template yang tersedia.</p></div>';
                    }
            ...
            error: function () {
                $("#template-sticky-body").html(
                    '<div class="text-center text-danger py-5">Gagal memuat template.</div>',
                );
            },
        });
    };
    ...
    window.filterTemplates = function () {
        var query = $("#search-templates").val().toLowerCase();
        $(".template-container-item").each(function () {
            var searchData = $(this).find(".template-grid-item").data("search");
        ...
        // Hide categories if all items are hidden
        $(".category-wrapper").each(function () {
            var hasVisible = $(this).find(".template-container-item:not(.d-none)").length > 0;
            if (hasVisible) {
                $(this).removeClass("d-none");
            } else {
                $(this).addClass("d-none");
    ...
    $(document).on("click", function (e) {
        if (
            ...
            !$(e.target).closest('button[onclick="toggleTemplatePreview()"]')
                .length
        ) {
            $("#template-sticky-container").addClass("d-none");
        }
    ...
    var initActions = function () {
        ...
        $(document).on("click", "#btn-refresh-group", function () {
            var customerId = $("#current-customer-id").val();
            var $btn = $(this);
            var $icon = $btn.find("i");
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        toastr.success("Info grup berhasil diperbarui");
                        // Update UI
                        if (response.name) {
                            $("#chat-customer-name").text(response.name);
                        ...
                        if (response.avatar_url) {
                            ...
                            $sidebarItem.find("img")
                                .attr("src", response.avatar_url)
                                .removeClass("d-none");
                            $sidebarItem.find(".symbol-label")
                                .addClass("d-none");
                        }
                    } else {
                        toastr.error(response.error || "Gagal memperbarui info grup");
                    }
                },
                error: function () {
                    toastr.error("Terjadi kesalahan sistem");
        ...
        $("#btn-save-rename-chat").on("click", function () {
            ...
            $.post(
                ...
                function (response) {
                    if (response.success) {
                        var updatedName = getDisplayName(
                            response.data || { display_name: name },
                            name,
                        );
                        ...
                        if (
                            ...
                            if (
                                !$("#chat-right-sidebar").hasClass("d-none") &&
                                type === "individual"
                            ) {
        ...
        $.post(
            ...
            function (response) {
                if (response.success) {
                    ...
                    if (
                        response.has_next &&
                        response.next_offset !== undefined
                    ) {
            ...
            success: function (response) {
                if (
                    ...
                    $('#label-checkbox-list input[type="checkbox"]').each(function () {
                        if (activeLabels.includes(String($(this).val()))) {
                    ...
                    if (response.success && response.data) {
                        if (
                            ...
                            response.data.forEach(function (msg) {
                                ...
                                if (
                                    msg.updated_at &&
                                    msg.updated_at > lastMessageDate
                                ) {
        ...
        $.post(
            ...
            function (response) {
                ...
                if (response.success) {
                    ...
                    toastr.error(
                        response.message || "Gagal sinkronisasi kontak",
                    );
                    finishSync(btn, icon, originalIconClass, false);
                }
            },
    ...
    var renderCustomersList = function (customers, replace = true) {
        ...
        customers.forEach(function (c) {
            ...
            if (hasUnread) {
                ...
                unreadHtml =
                    '<div class="unread-badge-wrapper ms-2 d-flex flex-column align-items-end">' +
                    '<span class="badge badge-circle badge-success whatsapp-green w-18px h-18px fs-9">' +
                    unreadCount +
                    "</span>" +
                    "</div>";
            }
            ...
            // Icon helper for groups and personal
            var avatarHtml = "";
            if (type === "group") {
                if (c.avatar_url) {
                    avatarHtml =
                    ...
                    avatarHtml =
                        '<div class="symbol symbol-50px symbol-circle">' +
                        '<div class="symbol-label fs-2 fw-bold" style="background-color: #6f42c1; color: white;">' + // Purple for groups
                ...
                if (c.avatar_url) {
                    ...
                    avatarHtml =
                        '<div class="symbol symbol-50px symbol-circle">' +
                        '<div class="symbol-label fs-2 fw-bold" style="background-color: #00a884; color: white;">' +
            ...
            html +=
                ...
                '<span class="' +
                nameClass +
                ' name-text text-truncate" style="max-width: 130px;">' +
                ...
                '<span class="' +
                msgClass +
                ' last-msg-text fs-7 text-truncate" style="max-width: 180px;">' +
                lastMsgHtml +
                "</span>" +
                unreadHtml +
                "</div>" +
    ...
    window.scrollToMessage = function (messageIdWa, attempt) {
        ...
        var target = $("#messages-list")
            ...
            .filter(function () {
                return (
                    String($(this).attr("data-message-id-wa") || "") ===
                    targetMessageId
                );
            })
            .first();
            ...
            loadPreviousMessages({
                ...
                onComplete: function (loaded) {
                    if (loaded) {
                        ...
                        toastr.info(
                            "Pesan asal tidak ditemukan di chat yang dimuat",
                        );
                    }
                },
            });
    ...
    var formatLastMessage = function (message, fallbackPhone) {
        ...
        // Handle Group Sender Prefix for cleaner preview
        if (text.indexOf("[SENDER:") !== -1) {
            text = text.replace(/\[SENDER:(.*?)\]\s*/, "$1: ");
        }
    ...
    var refreshAvatar = function (customerId, type = "individual") {
        $.ajax({
            url: appUrl + "chat/refresh_avatar",
            type: "POST",
            data: {
                customer_id: customerId,
                type: type,
            ...
            success: function (response) {
                if (response.success && response.avatar_url) {
                    // Update header if still on same customer
                    if (
                        currentCustomerId == customerId &&
                        currentCustomerType == type
                    ) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success && res.data) {
                    var c = res.data;
                    $("#sidebar-name").text(
                        getDisplayName(c, c.wa_number || "Tanpa Nama"),
                    );
                    ...
                    // Render labels
                    var labelsHtml = '<span class="text-muted fs-8">-</span>';
                    if (c.labels && c.labels.length > 0) {
                        labelsHtml = "";
                        c.labels.forEach(function (label) {
                            labelsHtml +=
                                '<span class="badge badge-light-primary me-1 mb-1" style="background-color: ' +
                                label.color +
                                '20; color: ' +
                                label.color +
                                '; border: 1px solid ' +
                    ...
                    $("#sidebar-labels").html(labelsHtml);
    ...
    var initDealModal = function () {
        ...
        if ($.fn.select2) {
            ...
            $("#deal-assigned").select2({
                dropdownParent: $("#modal-deal"),
                minimumResultsForSearch: 10,
            });

            if (typeof flatpickr !== "undefined") {
                $("#deal-followup").flatpickr({
                    enableTime: true,
                    dateFormat: "Y-m-d H:i",
                });
                $("#deal-close").flatpickr({
                    dateFormat: "Y-m-d",
                });
            }
        }
        ...
        $("#form-deal").on("submit", function (e) {
            ...
            var id = $("#deal-id").val();
            var url = id ? appUrl + "deals/update/" + id : appUrl + "deals/store";
    ...
    var initMessageActions = function () {
        ...
        $(document).on("contextmenu", ".message-bubble", function (e) {
            ...
            var wrapper = messageContextTarget.closest(
            ...
            var rawContent =
                messageContextTarget.attr("data-raw-content") || "";

            // If it's a deleted message, don't show context menu
            if (messageContextTarget.find(".message-deleted-text").length > 0)
                return;
            ...
            if (
                ...
                var now =
                    Math.floor(new Date().getTime() / 1000) + serverTimeOffset;
        ...
        $(document).on("click", "#msg-menu-reply", function (e) {
            ...
            if (content) {
                var replyType =
                    $("#message-context-menu").data("type") || "text";
                var cleanContent = getCleanMessageContent(content);
        ...
        $(document).on("click", "#msg-menu-forward", function (e) {
            ...
            if (
                ...
                toastr.warning(
                    "Pesan ini belum tersinkronisasi dengan WhatsApp dan tidak dapat diteruskan.",
                );
                $("#message-context-menu").hide();
                return;
            }
            ...
            var senderName = wrapper.hasClass("message-out")
                ? "Anda"
                : "Customer";
            var senderLabel = wrapper.find(".message-sender");
        ...
        $(document).on("click", "#msg-menu-edit", function (e) {
            ...
            if (
                ...
                toastr.warning(
                    "Pesan ini belum tersinkronisasi dengan WhatsApp dan tidak dapat diedit.",
                );
                $("#message-context-menu").hide();
                return;
            }
        ...
        $(document).on("click", ".forward-target-item", function () {
            ...
            var existingIndex = selectedForwardTargets.findIndex(
                function (target) {
                    return (
                        String(target.id) === targetId &&
                        String(target.type) === targetType
            ...
            if (existingIndex >= 0) {
                ...
                selectedForwardTargets.push({
                    ...
                    type: targetType,
                    isCurrentChat:
                        String(btn.attr("data-current-chat")) === "1",
                });
            }
        ...
        $("#btn-forward-selected").on("click", function () {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    target_chat_ids: selectedForwardTargets.map(
                        function (target) {
                            return target.id;
                        },
                    ),
                ...
                success: function (response) {
                    if (response.success) {
                        toastr.success(
                            response.message || "Pesan berhasil diteruskan",
                        );
                        ...
                        if (
                            response.failed_targets &&
                            response.failed_targets.length > 0
                        ) {
                            toastr.warning(
                                response.failed_targets.length +
                                    " target gagal dikirim",
                            );
                        }
                    } else {
                        toastr.error(
    ...
    const EMOJI_CATEGORIES = [
        ...
        },
        {
            ...
            icon: "❤️",
            emojis: [
                "💘", "💝", "💖", "💗", "💓", "💞", "💕", "💟", "❣️", "💔", "❤️", "🧡", "💛", "💚", "💙", "💜", "🤎", "🖤", "🤍", "💯", "💢", "💥", "💫", "💦", "💨", "🕳️", "💣", "💬", "👁️‍🗨️", "🗨️", "🗯️", "💭", "💤", "♨️", "🛑", "🕛", "🕧", "🕐", "🕜", "🕑", "🕝", "🕒", "🕞", "🕓", "🕟", "🕔", "🕠", "🕕", "🕡", "🕖", "🕢", "🕗", "🕣", "🕘", "🕤", "🕙", "🕥", "🕚", "🕦", "🌀", "♠️", "♥️", "♦️", "♣️", "🃏", "🀄", "🎴", "🎭", "🔇", "🔈", "🔉", "🔊", "📢", "📣", "📯", "🔔", "🔕", "🎼", "🎵", "🎶", "💹", "🏧", "🚮", "🚰", "♿", "🚹", "🚺", "🚻", "🚼", "🚾", "⚠️", "🚸", "⛔", "🚫", "🚳", "🚭", "🚯", "🚱", "🚷", "📵", "🔞", "☢️", "☣️", "⬆️", "↗️", "➡️", "↘️", "⬇️", "↙️", "⬅️", "↖️", "↕️", "↔️", "↩️", "↪️", "⤴️", "⤵️", "🔃", "🔄", "🔙", "🔚", "🔛", "🔜", "🔝", "🛐", "⚛️", "🕉️", "✡️", "☸️", "☯️", "✝️", "☦️", "☪️", "☮️", "🕎", "🔯", "♈", "♉", "♊", "♋", "♌", "♍", "♎", "♏", "♐", "♑", "♒", "♓", "⛎", "🔀", "🔁", "🔂", "▶️", "⏩", "⏭️", "⏯️", "◀️", "⏪", "⏮️", "🔼", "⏫", "🔽", "⏬", "⏸️", "⏹️", "⏺️", "⏏️", "🎦", "🔅", "📳", "📴", "🅰️", "🆎", "🅱️", "🆑", "🆒", "🆓", "ℹ️", "🆔", "Ⓜ️", "🆕", "🆖", "🅾️", "🆗", "🅿️", "🆘", "🆙", "🆚", "🈁", "🈂️", "🈷️", "🈶", "🈯", "🉐", "🈹", "🈚", "🈲", "🉑", "🈸", "🈴", "🈳", "㊗️", "㊙️", "🈺", "🈵", "🔴", "🟠", "🟡", "🟢", "🔵", "🟣", "🟤", "⚫", "⚪", "🟥", "🟧", "🟨", "🟩", "🟦", "🟪", "🟫", "⬛", "⬜", "📁", "📂", "🗂️"
            ]
        },
        {
            id: "flags",
    ...
    const initEmojiPickerOnce = function () {
        if (emojiPickerInitialized) return;

        // JoyPixels 6.0+ Configuration
        if (typeof joypixels !== "undefined") {
            joypixels.imagePathPNG = "https://cdn.jsdelivr.net/joypixels/assets/6.0/png/unicode/64/";
        }

        const categoriesContainer = document.querySelector(".emoji-categories");
        const listContainer = document.getElementById("emoji-list-container");
        ...
        EMOJI_CATEGORIES.forEach((cat, index) => {
            ...
            if (typeof joypixels !== "undefined") {
                catIcon = joypixels.toImage(cat.icon);
            }
            categoriesHtml += `<span class="category-icon cursor-pointer fs-3 p-1 ${
                index === 0 ? "active" : ""
            }" data-cat="${cat.id}" title="${cat.name}">${cat.icon}</span>`;
        });
        categoriesContainer.innerHTML = categoriesHtml;

        // Initial Load (Smileys)
        renderEmojiCategory("smileys");

        // Category Click Handler
        $(document).on("click", ".category-icon", function () {
            const catId = $(this).data("cat");
            $(".category-icon").removeClass("active border-bottom border-primary");
            $(this).addClass("active border-bottom border-primary");
            renderEmojiCategory(catId);
        });

        emojiPickerInitialized = true;
    };

    const renderEmojiCategory = function (catId) {
        const listContainer = document.getElementById("emoji-list-container");
        const category = EMOJI_CATEGORIES.find((c) => c.id === catId);
        if (!category) return;

        let html = `<div class="fw-bold text-muted fs-8 mb-3 uppercase">${category.name}</div>`;
        html += '<div class="d-flex flex-wrap gap-1">';
        category.emojis.forEach((emoji) => {
            // Using JoyPixels to render if available
            let emojiHtml = emoji;
            if (typeof joypixels !== "undefined") {
                emojiHtml = joypixels.toImage(emoji);
            }
            html += `<span class="emoji-item cursor-pointer p-1 hover-scale" onclick="insertEmoji('${emoji}')" style="line-height: 1;">${emojiHtml}</span>`;
        });
        html += "</div>";
    ...
    const saveRecentEmoji = function (emoji) {
        let recent = JSON.parse(localStorage.getItem("recent_emojis") || "[]");
        recent = [emoji, ...recent.filter((e) => e !== emoji)].slice(0, 16);
        localStorage.setItem("recent_emojis", JSON.stringify(recent));
    };

    window.renderRecentEmojis = function () {
        const recent = JSON.parse(localStorage.getItem("recent_emojis") || "[]");
        const container = document.getElementById("recent-emojis-container");
        if (!container) return;

        if (recent.length === 0) {
            $(".recent-section").addClass("d-none");
            return;
        }

        $(".recent-section").removeClass("d-none");
        let html = "";
        recent.forEach((emoji) => {
            ...
            html += `<span class="cursor-pointer p-1" onclick="insertEmoji('${emoji}')" style="line-height: 1;">${emojiHtml}</span>`;
        });
        container.innerHTML = html;
    };

    // Update parseMessageContent to include JoyPixels
    var originalParseMessageContent = parseMessageContent;
    parseMessageContent = function (body, type, message) {
        let content = originalParseMessageContent(body, type, message);
        if (typeof joypixels !== "undefined") {
            joypixels.imagePathPNG = "https://cdn.jsdelivr.net/joypixels/assets/6.0/png/unicode/64/";
            return joypixels.toImage(content);
        }
        return content;
    };
